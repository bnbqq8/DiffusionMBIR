...
from util.img_utils import clear, clear_color
...
class GaussianDiffusion:
        ...
        for idx in pbar:
            ...
            if record:
                if idx % 10 == 0:
                    ...
                    plt.imsave(file_path, clear(img), cmap="gray")

        return img

    def p_sample_loop_wo_condition(self, model, x_start, record, save_root):
        """
        The function used for sampling from noise.
        """
        img = x_start
    ...
    def p_sample_loop_volumetric(
        ...
        The function used for sampling from noise.
            x_start        [1, 1, D,         H, W]
            T1 measurement [1, 1, D//factor, H, W]
            T2 measurement [1, 1, D,         H, W//factor]
        """
        img = x_start
        device = x_start.device
        final = torch.zeros_like(img)
        ind_to_be_gen_t1 = torch.arange(img.shape[2]) % factor != 0
        pbar = tqdm(list(range(self.num_timesteps))[::-1])
        # timestep iteration
        for idx in pbar:
            time = torch.tensor([idx] * img.shape[0], device=device)
            ...
            # T1 interpolation
            for slice_idx in range(x_start.shape[2]):
                # TODO for slice_mask.sum()==0,
                if slice_idx % factor == 0:
                    final[:, :, slice_idx, :, :] = measurement_t1[
                        :, :, slice_idx // factor, :, :
                    ].detach()
                else:
                    img_slice = img[:, :, slice_idx, :, :].requires_grad_()
                    out = self.p_sample(x=img_slice, t=time, model=model)
                    final[:, :, slice_idx, :, :] = out["sample"]

                #
                # noisy_measurement_t1 = self.q_sample(
                #     measurement_t1[:, :, slice, :, :], t=time
                # )
                # img_slice, distance = measurement_cond_fn(
                #     x_t=out["sample"],
                #     measurement=measurement_t1[:, :, slice, :, :],
                #     noisy_measurement=noisy_measurement_t1,
                #     x_prev=img_slice,
                #     x_0_hat=out["pred_xstart"],
                # )
                # img[:, :, slice, :, :] = img_slice.detach()
