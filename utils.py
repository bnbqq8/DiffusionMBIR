...
def cal_metric(comp, label, data_range=None):
    LoG = functools.partial(gaussian_laplace, sigma=1.5)
    psnr_val = peak_signal_noise_ratio(comp, label, data_range=data_range)
    ssim_val = structural_similarity(comp, label, data_range=data_range)
